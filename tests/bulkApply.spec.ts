...
describe("Bulk Apply Tests", () => {
  it("should replace all occurrences of a given placeholder in a string", async () => {
    const stringToReplace = "Hello, I am {{user}}. I am {{user}}. I am {{user}}.";
    const user = os.userInfo().username;
    expect(await bulkApply(stringToReplace)).toBe(`Hello, I am ${user}. I am ${user}. I am ${user}.`);
  });

  it("should replace all occurrences of multiple placeholders in a string", async () => {
    const stringToReplace = "Hello, I am {{user}}. My timezone is {{timezone}}. Today is {{day}}.";
    const user = os.userInfo().username;
    const timezone =
    ...
    expect(await bulkApply(stringToReplace)).toBe(`Hello, I am ${user}. My timezone is ${timezone}. Today is ${day}.`);
  });
});
