...
const WeekRemindersPlaceholder: Placeholder = {
  ...
  apply: async (str: string, context?: { [key: string]: unknown }) => {
    if (context && "weekReminders" in context) {
      return {
        result: context["weekReminders"] as string,
        weekReminders: context["weekReminders"],
      };
    }

    const reminders = await EventFetcher.getUpcomingEvents("reminder", 1);
    return { result: reminders, weekReminders: reminders };
  },
  result_keys: ["weekReminders"],
  constant: true,
  fn: async () =>
    (await WeekRemindersPlaceholder.apply("{{weekReminders}}")).result,
  example:
    "Tell me about my reminders this week based on the following list: {{weekReminders}}.",
  description:
