...
const validatePlaceholderJSON = (jsonObject: JSONObject): [boolean, string] => {
  ...
  if (!jsonObject.hasOwnProperty("regex")) return [false, "Placeholder is missing regex field."];
  if (!jsonObject.hasOwnProperty("value")) return [false, "Placeholder is missing value field."];

  if (typeof jsonObject.name !== "string") return [false, "Placeholder name is not a string."];
  if (typeof jsonObject.regex !== "string") return [false, "Placeholder regex is not a string."];
  if (typeof jsonObject.value !== "string") return [false, "Placeholder value is not a string."];
  ...
  if (
    ...
    (jsonObject.categories as unknown[]).some((category) => typeof category !== "number")
  )
 ...
 */
export const loadPlaceholderFromJSONString = (jsonString: string): Placeholder | undefined => {
  try {
    const newPlaceholderData = JSON.parse(jsonString);

    const validity = validatePlaceholderJSON(newPlaceholderData);
    if (!validity[0]) throw `Error loading placeholder from JSON string. ${validity[1]}`;
    ...
    const newPlaceholder: Placeholder = {
      ...
      constant: true,
      fn: async (content: string) => (await newPlaceholder.apply(`{{${newPlaceholderData.name}}}`)).result,
    ...
    if (e instanceof SyntaxError) throw `Error loading placeholder from JSON string. Invalid JSON string: ${jsonString}`;
    throw e;
  }
};
 ...
 */
export const loadPlaceholdersFromJSONString = (jsonString: string): Placeholder[] => {
  try {
    const newPlaceholdersData = JSON.parse(jsonString);
    const newPlaceholders = (Object.entries(newPlaceholdersData) as [string, CustomPlaceholder][]).map(
      ([key, placeholder]) => {
        ...
        const newPlaceholder: Placeholder = {
          ...
          constant: true,
          fn: async (content: string) => (await newPlaceholder.apply(`{{${key}}}`)).result,
    ...
    if (e instanceof SyntaxError) throw `Error loading placeholders from JSON string. Invalid JSON string: ${jsonString}`;
    throw e;
  }
};
 ...
 * @returns An array of placeholder objects.
 */
export const loadPlaceholdersFromFile = async (filepath: string): Promise<Placeholder[]> => {
  try {
    const fileContents = await fs.promises.readFile(filepath, "utf-8");
    return loadPlaceholdersFromJSONString(fileContents);
  } catch (e) {
    if (e instanceof SyntaxError) throw `Error loading placeholders from file. Invalid JSON file: ${filepath}`;
    throw e;
  }
};
