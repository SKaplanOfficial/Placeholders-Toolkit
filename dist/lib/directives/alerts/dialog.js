 ...
 */
const DialogDirective = {
    name: "displayDialog",
    regex: /{{dialog( input=(true|false))?( timeout=([0-9]+))?( title="(([^{]|{(?!{)|{{[\s\S]*?}})*?)")?:(([^{]|{(?!{)|{{[\s\S]*?}})+?)}}/g,
    apply: async (str) => {
        const matches = str.match(/{{dialog( input=(true|false))?( timeout=([0-9]+))?( title="(([^{]|{(?!{)|{{[\s\S]*?}})*?)")?:(([^{]|{(?!{)|{{[\s\S]*?}})+?)}}/);
        if (matches) {
            const input = matches[2] == "true";
            const timeout = parseInt(matches[4]) || 30;
            const title = matches[6] || "Pins";
            const message = matches[8];
            const result = await (0, utils_1.runAppleScript)(`display dialog "${message.replaceAll('"', "'")}" with title "${title.replaceAll('"', "'")}"${input ? ' default answer ""' : ""} giving up after ${timeout}`, { timeout: timeout * 1000 });
            if (input) {
    ...
    constant: false,
    fn: async (message, title, timeout, input = "false") => (await DialogDirective.apply(`{{dialog${input ? ` input=${input}` : ""}${timeout ? ` timeout=${timeout}` : ""}${title ? ` title="${title}"` : ""}:${message}}}`)).result,
    example: '{{dialog title="Info":Hello World}}',
    description: "Directive to display a dialog message with an optional title, timeout, and/or input field. If no timeout is provided, the alert will timeout after 10 seconds. If the input setting is `true`, the placeholder will be replaced with the user's input. Otherwise, the placeholder will be replaced with an empty string.",
...
exports.default = DialogDirective;
