...
const ShellScriptPlaceholder = {
    ...
    apply: async (str) => {
        try {
            ...
            if (!script)
                return { result: "", shell: "" };
            const res = (0, child_process_1.execSync)(script, { encoding: "ascii", shell: bin }).toString().trim();
    ...
    constant: false,
    fn: async (script, bin = "/bin/zsh") => (await ShellScriptPlaceholder.apply(`{{shell ${bin}:${script}}}`))
        .result,
